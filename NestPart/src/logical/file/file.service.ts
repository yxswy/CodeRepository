...
export class FileService {
  ...
  constructor(
    ...
    //           await this.processLineByLine(filePath)
  ...
  async getFileList() {
    const res: any[] = await this.itemRepository.query(
      'SELECT id,file_name,file_path,file_extname,file_title,readings,file_content,date_format(update_time,"%Y-%m-%d") as update_time,date_format(create_time,"%Y-%m-%d") as create_time FROM file_entity',
    )
    return {
      data: {
        data_type: 'list',
  ...
  async saveData({ fileName, fileTitle, fileContent, fileExtname, filePath }) {
    await this.itemRepository.query(
      'INSERT INTO file_entity (file_name, file_path, file_extname, file_title, file_content)  VALUES  (?, ?, ?, ?, ?);',
      [fileName, filePath, fileExtname, fileTitle, fileContent],
    )
